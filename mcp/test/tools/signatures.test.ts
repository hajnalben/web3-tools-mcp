  ...
  describe('get_event_signature', () => {
    it('should generate correct topic0 for Transfer event', async () => {
      const result = await signatureTools.get_event_signature.handler({
        items: [{ eventAbi: 'event Transfer(address indexed from, address indexed to, uint256 value)' }]
      })

      const data = JSON.parse(result.content[0].text)
      expect(data).toHaveLength(1)
      expect(data[0].topic0).toBe('0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef')

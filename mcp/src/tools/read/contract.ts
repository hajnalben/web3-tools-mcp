...
const ContractCallSchema = z.object({
  ...
  contractAddress: z.string().describe('Contract address to call (must be valid checksummed address)'),
  functionAbi: z
    .string()
    .describe(
  ...
  blockNumber: z.string().optional().describe('Block number for historical queries (defaults to latest)'),
...
export default {
  ...
  is_contract: createTool(
    ...
    z.object({
      ...
      address: z.string().describe('Address to check (returns true if contract, false if EOA/wallet)')
    }),
    async (args) => {
